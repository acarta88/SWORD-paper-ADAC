# ==============================================================================
# FULL BENCHMARK GENERATION — ODRF baseline on the 248 real datasets
...
#
# Recipe: ODRF::ODRF(target ~ ., data = df_train, split = "mse",
# parallel = FALSE, ntrees = 100, storeOOB = FALSE) with the hand-written
# predict methods in odrf_predict_patch.R (the package's own predict did not
# work correctly for this regression setting).
# ==============================================================================
...
foreach(
  ...
  tryCatch({
    ...
    result_file <- file.path(DIR_RESULTS, paste0("risultati_ODRF_ADAC_", safe_ds, "_", start_rep, "_", end_rep, ".rds"))
    ...
    for (rep in start_rep:end_rep) {
      ...
      df_test  <- df_test[,  !constant_cols, drop = FALSE]
      ...
      pred_matrix_all <- predict.ODRF(model, df_test[, -which(names(df_test) == "target"), drop = FALSE], type = "tree")
      ...
      saveRDS(as.numeric(pred_test), file.path(DIR_PRED, paste0("pred_ODRF_", safe_ds, "_rep", rep, ".rds")))
      ...
      risultati_finali[[rep]] <- data.frame(
        ...
        time_forest = time_forest, mtry = "ODRF", ntree = 100
      )
    }
...
foreach(
  ...
  tryCatch({
    ...
    all_done <- all(sapply(start_rep:end_rep, function(r)
      file.exists(file.path(DIR_PERTURB, paste0("pred_perturbed_ODRF_", safe_ds, "_rep", r, ".rds")))))
    ...
    for (rep in start_rep:end_rep) {
      out_file <- file.path(DIR_PERTURB, paste0("pred_perturbed_ODRF_", safe_ds, "_rep", rep, ".rds"))
