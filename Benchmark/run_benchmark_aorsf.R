# ==============================================================================
# FULL BENCHMARK GENERATION — Accelerated Oblique Random Survival Forest,
# regression mode with GLM node models (aorsf) baseline on the 248 real
...
# control = orsf_control_regression(method = "glm")). See run_benchmark_aorsfNET.R
# for the "net" (penalized) node-model variant.
# ==============================================================================
...
foreach(
  ...
  tryCatch({
    ...
    result_file <- file.path(DIR_RESULTS, paste0("risultati_aorsf_ADAC_", safe_ds, "_", start_rep, "_", end_rep, ".rds"))
    ...
    for (rep in start_rep:end_rep) {
      ...
      model <- orsf(data = df_train, formula = target ~ ., n_tree = 100,
                    control = orsf_control_regression(method = "glm"))
      ...
      saveRDS(as.numeric(pred_test), file.path(DIR_PRED, paste0("pred_aorsf_", safe_ds, "_rep", rep, ".rds")))
      ...
      risultati_finali[[rep]] <- data.frame(
        ...
        time_forest = time_forest, ntree = 100, mtry = "aorsf"
      )
    }
...
foreach(
  ...
  tryCatch({
    ...
    all_done <- all(sapply(start_rep:end_rep, function(r)
      file.exists(file.path(DIR_PERTURB, paste0("pred_perturbed_aorsf_", safe_ds, "_rep", r, ".rds")))))
    ...
    for (rep in start_rep:end_rep) {
      out_file <- file.path(DIR_PERTURB, paste0("pred_perturbed_aorsf_", safe_ds, "_rep", rep, ".rds"))
