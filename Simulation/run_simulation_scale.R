...
# caches actually shipped (results_cache/df_SWORD72_cached.rds and
# risultati_full_simulation_scale/VI_SWORD_seed1_scale_cached.rds) are what
# the paper's figures/tables read from — this script is NOT required to
# reproduce any of them (see tab1_wilcox_nrmse.R, tab3_tabA1_wilcox_corstrength.R,
# fig2_tabA2_compute_time.R, fig1_variable_importance.R). Run this only to regenerate
# everything from the raw simulated datasets.
# ==============================================================================
rm(list = ls()); gc()
...
check_progress <- function(cartella = file.path(SCRIPT_DIR, "risultati_full_simulation_scale")) {
...
#   large (n_obs*features >= 10000): few workers to keep RAM in check
#   small: max workers
n_workers_large <- min(60, max(1, n_core_totali - 16))
n_workers_small <- min(60, max(1, n_core_totali - 16))
cartella_scale  <- file.path(SCRIPT_DIR, "risultati_full_simulation_scale")
log_file        <- file.path(cartella_scale, "progress_log.txt")
checkpoint_file <- file.path(cartella_scale, "checkpoint.txt")
...
cat("============================================================\n")
cat("  SWORD SIMULATION -- Weight_Scheme = scale\n")
...
if (!dir.exists(cartella_scale)) dir.create(cartella_scale, recursive = TRUE)
...
# Split by size: large (n_obs * features >= 10000) vs small
da_fare_grandi  <- Filter(function(x) x$n_obs * x$features >= 10000, da_fare)
da_fare_piccoli <- Filter(function(x) x$n_obs * x$features <  10000, da_fare)
cat(sprintf("  Large (n*p >= 10000): %d | Small: %d\n\n",
...
if (.Platform$OS.type == "windows") {
  ...
  .setup_cluster <- function(cl) {
    ...
    clusterExport(cl, varlist = c(funzioni_helper,
                                  "cartella_scale", "log_file", "checkpoint_file",
...
cat("========================================\n")
cat("STARTING SWORD SIMULATION (scale)\n")
...
.process_item <- function(item) {
  ...
  # --- Output file path ---
  file_name <- paste0("scale__", gsub("\\$", "__", substr(path, 2, 999)))
  rds_path  <- file.path(cartella_scale, paste0(file_name, ".rds"))
...
# ==============================================================================
# PART 8: AGGREGATION -- df_finale_SWORD_scale
# ==============================================================================

cat("Aggregating results into a data frame...\n")

file_rds <- list.files(cartella_scale, pattern = "\\.rds$", full.names = TRUE)
file_rds <- file_rds[basename(file_rds) != "risultati_completi.rds"]

df_finale_SWORD_scale <- do.call(rbind, lapply(file_rds, function(f) {
...
cat("Rows in df_finale_SWORD_scale:", nrow(df_finale_SWORD_scale), "\n")

save(df_finale_SWORD_scale,
     file = file.path(cartella_scale, "df_finale_SWORD_scale.rda"))
cat("Saved: risultati_full_simulation_scale/df_finale_SWORD_scale.rda\n")
cat("\nDONE.\n")
